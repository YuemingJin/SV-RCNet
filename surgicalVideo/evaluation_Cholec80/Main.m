...
for k = 41:80
    num = num2str(k);
    to_add = ['video' num];
    to_add = [to_add '-phase.txt'];
    phaseGroundTruths = [phaseGroundTruths to_add];
end
...
fps = 1;
% fpg=1 in Cholec80 dataset

for i = 1:length(phaseGroundTruths)
    gt_rootfolder = 'gt-phase/';
    ...
    predFile = [model_rootfolder phaseGroundTruth_single(1:end-10) 'name.txt'];
